...
- **Ask your coding agent to do the same.** `AGENTS.md` carries the working
  ...
  as a program — also what `npm run check` runs).

**Treat the starters as scratch paper.** They ship approved by
`ksor-starter/0.0.48` — a tool, not a person. Edit the body and that
...
with `curl`. With `npm run serve` running, write `.mcp.json` at the repo root:
...
an empty shelf. They are approved by `ksor-starter/0.0.48`: the tool
...
3. **When the last one is gone, delete `ksor-starter/0.0.48` from
...
| `npm run preview` exits `3`                                             | there is no `system/site/out/` yet                                                                         | run the build first                                                                    |
