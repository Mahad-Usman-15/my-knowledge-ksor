...
```sh
npm run check     # refuses an orphan deck
npm run dev       # look at the page — the deck renders after the introduction,
               # immediately before the first `##` section
```

`npm run build` refuses — the deck's own shape is checked in the site build, not
by `npm run check`:
